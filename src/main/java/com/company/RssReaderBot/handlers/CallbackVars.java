package com.company.RssReaderBot.handlers;

public class CallbackVars {
    // callback data fields
    public static final String START_MENU = "start_menu";
    public static final String RSS_FAQ = "rss_faq";
    public static final String MAIN_MENU = "main_menu";
    public static final String LOAD_ALL_ITEMS = "load_all_items";
    public static final String LOAD_BY_TITLE = "load_by_title";
    public static final String LOAD_BY_DATE = "load_by_date";
    public static final String PERSONAL_MENU = "personal_menu";
    public static final String FAVORITES = "user_favorites";
    public static final String CHANGE_RSS_URL = "change_rss_url";
    public static final String ALERTS_AND_TITLE_CONF = "alerts_title_items";
    public static final String UPDATE_USER_DATA = "update_user_data";
    public static final String RETURN_LOAD_BY_TITLE = "load_by_title_inline";
    public static final String SELECTED_BY_TITLE_CALLBACK = "EP "; // custom marker
    public static final String NEXT_PAGE = "next_page";
    public static final String PREVIOUS_PAGE = "previous_page";
    public static final String FIRST_PAGE = "first_page";
    public static final String LAST_PAGE = "last_page";
}
